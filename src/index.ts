...
function readGitState(cwd: string, recentCommits: number): GitState {
  const empty: GitState = { inRepo: false, branch: '', dirtyFiles: 0, stagedFiles: 0, lastCommit: '' }
  ...
  if (!branch) return empty // 非 git 仓库或无 HEAD → 不注入
...
function formatGitState(s: GitState): string {
  return [
...
export function apply(ctx: Context, config?: Config): void {
  ...
  const systemPrompt = (ctx as unknown as {
    systemPrompt: {
      context(opts: { name: string; order: number; text: string | (() => string) }): () => void
  ...
  ctx.logger.info(
    `[dsh-repo-context] ${state.inRepo ? `注入 git 上下文: branch=${state.branch} dirty=${state.dirtyFiles}` : '非 git 仓库，仅注入规范指引'}`,
  )
}
